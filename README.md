# Hi 👋 I'm Teumay Werashe

### Software Engineer — Full-Stack Developer

📍 Addis Ababa, Ethiopia
📞 +251 952 731 011
📧 [teumay21@gmail.com](mailto:teumay21@gmail.com)

[![GitHub](https://img.shields.io/badge/GitHub-Profile-black?style=for-the-badge\&logo=github)](https://github.com/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge\&logo=linkedin)](#)
[![Portfolio](https://img.shields.io/badge/Portfolio-Website-orange?style=for-the-badge\&logo=vercel)](#)

---

## 🚀 Professional Summary

Software Engineer and **Full-Stack Developer** with hands-on experience building scalable web applications using the **MERN stack**.

Experienced in designing **RESTful APIs, secure authentication systems, and responsive user interfaces**. Passionate about writing clean code, solving technical challenges, and building **high-performance startup-ready software solutions**.

---

# 🛠 Tech Stack

### Languages & Frameworks

![JavaScript](https://img.shields.io/badge/JavaScript-ES6+-yellow?style=for-the-badge\&logo=javascript)
![Node.js](https://img.shields.io/badge/Node.js-Backend-green?style=for-the-badge\&logo=node.js)
![Express.js](https://img.shields.io/badge/Express.js-API-black?style=for-the-badge\&logo=express)
![React](https://img.shields.io/badge/React-Frontend-blue?style=for-the-badge\&logo=react)
![HTML5](https://img.shields.io/badge/HTML5-Markup-orange?style=for-the-badge\&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-Styling-blue?style=for-the-badge\&logo=css3)

---

### Frontend Tools

![Redux](https://img.shields.io/badge/Redux_Toolkit-State-purple?style=for-the-badge\&logo=redux)
![Zustand](https://img.shields.io/badge/Zustand-State-black?style=for-the-badge)
![Context API](https://img.shields.io/badge/React_Context-State-blue?style=for-the-badge\&logo=react)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-UI-cyan?style=for-the-badge\&logo=tailwindcss)

---

### Backend & Databases

![MongoDB](https://img.shields.io/badge/MongoDB-Database-green?style=for-the-badge\&logo=mongodb)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Database-blue?style=for-the-badge\&logo=postgresql)
![Supabase](https://img.shields.io/badge/Supabase-Backend-green?style=for-the-badge\&logo=supabase)
![Express](https://img.shields.io/badge/Express-Server-black?style=for-the-badge\&logo=express)

---

### Tools & DevOps

![Git](https://img.shields.io/badge/Git-VersionControl-orange?style=for-the-badge\&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-Repository-black?style=for-the-badge\&logo=github)
![Docker](https://img.shields.io/badge/Docker-Container-blue?style=for-the-badge\&logo=docker)
![Postman](https://img.shields.io/badge/Postman-API-orange?style=for-the-badge\&logo=postman)
![Vercel](https://img.shields.io/badge/Vercel-Deployment-black?style=for-the-badge\&logo=vercel)
![Render](https://img.shields.io/badge/Render-Hosting-purple?style=for-the-badge)

---

# 📌 Selected Projects

### 🎓 Resource Hub – University Academic Platform

Full-Stack Web Application

• Built a platform aggregating academic resources for **45+ universities**
• Implemented **Role-Based Access Control (RBAC)** for verified administrators
• Designed advanced **CRUD systems** for managing academic resources

---

### 🏫 Campus Tour Assistance System

Web Application

• Developed a **digital campus guide** for visitors and prospective students
• Integrated an **AI chatbot** for answering campus questions
• Added interactive campus navigation features

---

### 🍔 Food Delivery Application

MERN Stack

• Built product menu browsing, cart logic, and order tracking
• Integrated **Chapa payment gateway** for Ethiopian transactions
• Created a powerful **Admin Dashboard** for order management

---

### 🛒 E-Commerce Platform

Web Application

• Developed a scalable **online shopping platform**
• Implemented **authentication, search, and checkout systems**
• Built an **admin panel for managing products and inventory**

---

### 🏢 Campus Facility Management System

Web Application

• Built an **issue reporting system** for campus infrastructure
• Implemented **real-time notifications** for updates
• Added a **collaborative comment system** for staff and students

---

### 🌐 Portfolio Website

• Built using **React.js and Framer Motion**
• Integrated **EmailJS** for contact form functionality
• Focused on **performance and modern UI animations**

---

# 🎓 Education

**B.Sc. in Software Engineering**
Addis Ababa University — Ethiopia
2023 – Present

Relevant Coursework:

• Data Structures & Algorithms
• Database Systems
• Computer Networks
• Operating Systems
• System Design
• Object-Oriented Programming (OOP)

---

# 🌍 Languages

English — Fluent
Amharic — Native
Tigrigna — Native

---

# 📊 GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME\&show_icons=true\&theme=github_dark)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME\&layout=compact\&theme=github_dark)

---

⭐ From [Teumay Werashe](https://github.com/YOUR_USERNAME)
